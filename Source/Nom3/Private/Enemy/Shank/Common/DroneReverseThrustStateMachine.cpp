...
UDroneReverseThrustStateMachine::UDroneReverseThrustStateMachine()
{
...
void UDroneReverseThrustStateMachine::EnterState()
{
	Super::EnterState();
	...
	//초기 속도 저장
	InitVelocity = OwnerDrone->DroneMoveComp->Velocity.Length();

	//역추진 제한 시간 초기화
	LimitTimeInState = 0.5;
}

void UDroneReverseThrustStateMachine::ExecuteState()
{
	Super::ExecuteState();
	...
	OwnerDrone->DroneMoveComp->ReverseVectorThrust(FMath::Abs(InAccel));

	//역추진 제한시간 초과
	if (ElapsedTimeInState > LimitTimeInState)
	{
		if (OwnerDrone->PathQueue.IsEmpty())
		{
			//추진력를 0으로 초기화해줘야 오류가 발생하지 않는다
			OwnerDrone->DroneMoveComp->ThrottleOff();
...
void UDroneReverseThrustStateMachine::ExitState()
{
	Super::ExitState();
